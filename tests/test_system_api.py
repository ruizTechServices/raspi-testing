...
from unified_server import app_factory as app_factory_module


def test_system_temperature_returns_cached_snapshot(client, monkeypatch):
    app_factory_module._pi_temperature_cache.update({
        'temperature_c': None,
        'temperature_f': None,
        'fetched_at': None,
    })
    ...
    monkeypatch.setattr(app_factory_module, '_read_pi_temperature_c', fake_read)
...
def test_system_temperature_classifies_warm_values():
    payload = app_factory_module._build_temperature_payload(73.2, 1234567890.0, cached=True, age_seconds=4.2)
...
def test_system_llm_status_returns_provider_checks(client, monkeypatch):
    monkeypatch.setattr(app_factory_module, '_probe_ollama_status', lambda: {
        'provider': 'ollama',
        'ok': True,
        'status': 'online',
        'detail': 'Reachable.',
    ...
    monkeypatch.setattr(app_factory_module, '_probe_openai_status', lambda: {
        'provider': 'openai',
        'ok': False,
        'status': 'quota',
        'detail': 'Out of quota.',
...
def test_ollama_control_runs_real_action(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(app_factory_module.subprocess, 'run', lambda *args, **kwargs: next(responses))
